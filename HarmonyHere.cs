    ...
    [HarmonyPatch(typeof(Pawn_NeedsTracker), "AddOrRemoveNeedsAsAppropriate", new Type[]
{
    ...
    public static class Harmony_Need_Remove
    {
        ...
        public static List<NeedDef> getUselessNeed()
        {
            ...
            result.Add(NeedDefOf.Food);
            result.Add(NeedDefOf.Rest);
            result.Add(NeedDefOf.Joy);
            result.Add(easyGetND("Beauty"));
            result.Add(easyGetND("Comfort"));
            result.Add(easyGetND("RoomSize"));
            result.Add(easyGetND("Outdoors"));
            result.Add(easyGetND("Mood"));
            return result;
        }
    ...
    [HarmonyPatch(typeof(Pawn_HealthTracker), "DropBloodFilth", new Type[]
{
        ...
        public static bool Prefix(HediffSet __instance, Hediff hediff, DamageInfo? dinfo = null, DamageWorker.DamageResult damageResult = null)
        {
            ...
            if (pawn != null && pawn.isWarframe())
            {
                if (hediff!=null && hediff.def.isBad)
                    if (hediff.Part!=null&&hediff.Part.def.defName != "wf_armor")
                    {
                        if (dinfo != null) { 
    ...
    [HarmonyPatch(typeof(PawnDiedOrDownedThoughtsUtility), "AppendThoughts_ForHumanlike", new Type[]{ typeof(Pawn),typeof(DamageInfo?),typeof(PawnDiedOrDownedThoughtsKind),typeof(List<IndividualThoughtToAdd>),typeof(List<ThoughtToAddToAll>)})]
    ...
    //warframe等级状态显示
    [HarmonyPatch(typeof(StatWorker), "ShouldShowFor", new Type[] {typeof(StatRequest)
    })]
    public static class Harmony_WarframeStatLevel
    {
        public static bool Prefix(StatWorker __instance, StatRequest req, ref bool __result)
        {
            Traverse tv = Traverse.Create(__instance);
            StatDef stat = tv.Field("stat").GetValue<StatDef>();
            Pawn pawn;
            if (stat!=null&& "Warframe_WarframeLevel".Equals(stat.defName)) {
