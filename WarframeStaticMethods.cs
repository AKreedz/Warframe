    ...
    public static class WarframeStaticMethods
    {
        ...
        */
        ...
        public static Pawn getWarframePawn(PawnKindDef pk) {
            ...
            PawnGenerationRequest request = new PawnGenerationRequest(pk, Faction.OfPlayer, PawnGenerationContext.NonPlayer, -1, 
                false, false, false, false, true, true, 0, false, true, true, false, false, false, false,false,0,0,null,1,
                null, null,null,null, 12, null, null, setGender(pk.defName), null);
            Pawn item = PawnGenerator.GeneratePawn(request);
        ...
        public static void showDamageAmount(Thing target,string damage) {
            if (ModLister.HasActiveModWithName("Damage Indicators [1.3]")) {
                return;
            }
        ...
        public static Gizmo getMulJump(Pawn pawn) {
            ...
            command_Target.action = delegate (Pawn p,LocalTargetInfo poc)
            {
                ...
                foreach (Pawn pawn2 in enumerable)
                {
                    ...
                    {
                        IntVec3 cell = poc.Cell;
                        Map map = pawn2.Map;
                        PawnFlyer pawnFlyer = PawnFlyer.MakeFlyer(ThingDef.Named("WarframeJumper"), pawn2, poc.Cell);
                        if (pawnFlyer != null)
                        {
                            GenSpawn.Spawn(pawnFlyer, cell, map, 0);
                            return;
                        }
                    }
                    else {
