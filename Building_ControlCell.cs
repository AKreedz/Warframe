...
namespace Warframe
{
    ...
    public class Building_ControlCell : Building_Casket,ISuspendableThingHolder
    {
        ...
        public override IEnumerable<FloatMenuOption> GetFloatMenuOptions(Pawn myPawn)
        {
            ...
            if (this.innerContainer.Count == 0)
            {
                ...
                if (!ReachabilityUtility.CanReach(myPawn, this, PathEndMode.InteractionCell, Danger.Deadly, false,false, TraverseMode.ByPawn))
                {
        ...
        public override IEnumerable<Gizmo> GetGizmos()
        {
            ...
            tar.action = delegate (LocalTargetInfo target)
            {
                if (target.Pawn!=null && (target.Pawn).isWarframe())
                {
                    if (this.becontroler != null || WFModBase.Instance._WFcontrolstorage.checkBeControlerExist(target.Pawn))
                    {
                        SoundStarter.PlayOneShotOnCamera(SoundDefOf.ClickReject, null);
                        return;
                    }
                    this.becontroler = (target.Pawn);
                    WFModBase.Instance._WFcontrolstorage.ControlSomeone(this, this.becontroler);
                }
            };
